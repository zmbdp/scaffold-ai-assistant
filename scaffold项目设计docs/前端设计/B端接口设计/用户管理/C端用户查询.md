...
- **查询 C 端用户列表**：B 端管理后台分页查询 C 端用户，支持按用户 ID、手机号、昵称、微信 openId（历史数据）等条件过滤。**B 端只能查询，不能新增/编辑/删除 C 端用户**（C 端用户新增由 C 端注册流程通过内部 Feign 调用完成，编辑由 C 端自身接口完成，均不在 B 端前端调用范围内）。

> C 端用户当前注册方式有两种：手机号注册、邮箱注册，均由 C 端注册流程内部调用，B 端不涉及。`openId` 字段为历史微信注册用户保留，当前版本不再支持微信注册。
>
...
| `openId` | String | 否 | — | `null` | 微信 openId（历史数据，当前版本不再支持微信注册） | 精确匹配（`open_id =`） |
...
| `openId` | String | 微信 openId（历史数据，当前版本不再支持微信注册；未绑定时为 `null`） |
...
C 端用户通过以下方式之一注册（由 C 端注册流程内部调用，B 端不直接调用）：

| 注册方式 | 标识字段 | 说明 |
| --- | --- | --- |
| 手机号注册 | `phoneNumber` 非空 | 手机号验证码登录后注册（当前支持） |
| 邮箱注册 | `email` 非空 | 邮箱登录后注册（当前支持） |
| 微信 openId 注册 | `openId` 非空 | 微信小程序/公众号登录后注册（**历史方式，当前版本不再支持**） |
